function RUN_1_makecorestacks_Validation(infolderloc, outfolderloc,basicfolderloc, folders, maxfield, cycles, tilesize, prefix1, wavelengths) 

tic
thr = max(tilesize);

for folder = 1:length(folders)
    if maxfield(folder)<100 %Changing dim to get correct file name
        dim = ['%02d'];
    ...
    for i3 = 1:length(cycles)
        for j1 = 1:4
            try
                DG{i3,j1} = double(imread([outfolderloc basicfolderloc num2str(cycles(i3)) '-dfp-basic.tif'],'Index',j1));  %
                FF{i3,j1} = double(imread([outfolderloc basicfolderloc num2str(cycles(i3)) '-ffp-basic.tif'],'Index',j1));  %
            catch
                [i3,j1]
