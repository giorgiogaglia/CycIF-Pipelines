function RUN_1_makecorestacks_z(infolderloc, outfolderloc,basicfolderloc, folders, maxfield, cycles, tilesize, prefix1, wavelengths, indx, indx1) 
...
for folder = 1:length(folders)
    ...
    for i1 = 1:length(prefix1)
        for i2 = 1:length(prefix2)
            ...
            try
                DAPICycle0 = imread([infolderloc filesep folders{folder} filesep folders{folder} '_ROUND_' num2str(cycles(1)) filesep prefix1{i1} num2str(prefix2(i2),dim) ' wv UV - DAPI).tif']);    %
            ...
            if prctile(DAPICycle0(:),97.5) < 100    %To skip fields that have no signal
                continue
            elseif exist([outfolderloc filesep folders{folder} filesep 'DAPIStacks' filesep 'DAPI_Field' num2str(prefix2(i2),dim) 'test.tif'], 'file') == 2
                continue
            end
            ...
            for i3 = 1:length(cycles)
                ...
                for j1 = 1:rows
                    for j2 = 1:cols
                        tiles_img{j1,j2} = imread([infolderloc filesep folders{folder} filesep folders{folder} '_ROUND_' num2str(cycles(i3)) filesep prefix1{i1} num2str(prefix2(i2),dim) ' wv UV - DAPI).tif']);   %
                ...
                for wv1 = 1:length(wavelengths)
                    ...
                    if flag == 1
                        if  i3 == indx1 && wv1 == indx %z-stacks for cycle 2 dsred %
                            %disp('z-stacks')
                            for z1 = 1:5
                                for j1 = 1:size(tile_col,1)
                                    for j2 = 1:size(tile_col,2)
                                        try
                                            temp_img{j1,j2} = imread([infolderloc filesep folders{folder} filesep folders{folder} '_ROUND_' num2str(cycles(i3)) filesep prefix1{i1} num2str(prefix2(i2),dim) ' '  wavelengths{wv1} ' z ' num2str(z1, '%1d') ').tif']);
                                        catch
                                            temp_mont=[];
                                            continue
                                        end
                        ...
                        elseif i3 ==indx1 && wv1~=indx
                            %skipping other wavelengths for round 1
                        else
                            for j1 = 1:size(tile_col,1)
                                for j2 = 1:size(tile_col,2)
                                    try
                                        temp_img{j1,j2} = imread([infolderloc filesep folders{folder} filesep folders{folder} '_ROUND_' num2str(cycles(i3)) filesep prefix1{i1} num2str(prefix2(i2),dim) ' '  wavelengths{wv1} ').tif']);   %
                                    catch
                                        continue
                                    end
                                    try
            ...
            final_stack = [outfolderloc filesep folders{folder} filesep 'FullStacks' filesep 'Field' num2str(prefix2(i2),dim1) 'test.tif']; %
            DAPI_stack  = [outfolderloc filesep folders{folder} filesep 'DAPIStacks' filesep 'DAPI_Field' num2str(prefix2(i2),dim1) 'test.tif'];    %
