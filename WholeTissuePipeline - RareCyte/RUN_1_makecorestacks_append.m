function RUN_1_makecorestacks_append(master_folderIN, basefolder,basicfolderloc, slides_folders, maxfields, cycles, tilesize, prefix1, wavelengths, z_wave, z_cycle,z_num, max_rows, well_nums , well_lets, DAPIwv)
    ...
    for i1 = 1:max_rows
        for i3 = 1:length(well_nums)
            for i2 = 1:length(prefix2)
                RunningStack = [];
                DAPIStack = [];
                if exist([basefolder filesep slides_folders{folder} filesep 'DAPIStacks' filesep  well_lets{i1} well_nums(i3) 'DAPI_Field' num2str(prefix2(i2),dim1) 'test.tif'], 'file') == 2
                    DAPICycle0 = imread([basefolder filesep slides_folders{folder} filesep 'DAPIStacks' filesep  well_lets{i1} well_nums(i3) 'DAPI_Field' num2str(prefix2(i2),dim1) 'test.tif'], 'Index',1);    %
                    ...
                    try
                        ...
                        DAPIStack = cat(3,DAPIStack,DAPICycle0);    %Putting DAPICycle0 as 1st in stack- 2nd image is same as 1st in stack if first cycle is the cycle you want everything compared to 
                ...
                saveastiff(RunningStack,final_stack, options);
                saveastiff(DAPIStack,DAPI_stack, options);
