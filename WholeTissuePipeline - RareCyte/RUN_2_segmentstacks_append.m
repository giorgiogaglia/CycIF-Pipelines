function RUN_2_segmentstacks_append(basefolder, slides_folders, maxfields,DAPIslice, options, max_rows, well_nums, well_lets, first_cycle)
        ...
        for i3 = 1:length(well_nums)
            for i2 = 1:length(prefix2)
                ...
                if exist(track_stack, 'file') == 2
                    try
                        SegImage1=imread(track_stack,'Index',1); %Get Segmented Image of first cycle
                        options.append = true;
                    catch
                        continue
                    end
                    ...
                    try
                        DAPIImage=imread(FileTif,'Index',DAPIslice(first_cycle)); %Run Segmentation for HSF1 cycle- Change DAPIslice to correct one for HSF1 cycle or 1st cycle
                ...
                if max(SegImage1(:)) > 0    %Checking that the field has cells
                    ...
                    saveastiff(uint16(TrackImage),track_stack, options); % track cells based off cycle 1
                end
            end
        end
    end
